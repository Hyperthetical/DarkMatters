...
def b_sanchez(M,Mmin,z,cosmo):
    alpha = -2.0
    A = 0.012
    mset = np.logspace(np.log10(Mmin),np.log10(M),num=50)
    c = cosmology.cvir_p12(mset,z,cosmo)
    cM = cosmology.cvir_p12(M,z,cosmo)
    f = np.log(1+c) - 1.0/(1+c)
    fM = np.log(1+cM)-1.0/(1+cM)
    L = 4.0*np.pi*mset*c**3/f**2
    LM = 4.0*np.pi*M*cM**3/fM**2
    dNdm = A/M*(mset/M)**alpha
    integ = L*2*dNdm
    return tools.Integrate(integ,mset)/LM

def b_p12(M,cosmo):
    bi = np.array([-0.442,0.0796,-0.0025,4.77e-6,4.77e-6,-9.69e-8])
    lm = np.log(M)
...
def rho_sub_dist(r_set,rho,rb,N,dmmod,alphas):
    ...
    if(dmmod != -1):
        n = len(r_set)
        rho_sub = np.zeros(n,dtype=float)
        for i in range(0,n):
            r = r_set[i]
...
def radio_power_ratio(halo,phys,sim):
    ...
    tset = np.zeros(ntheta,dtype=float) #angular sampling values
    ...
    for i in range(0,num):  #loop over freq
        ...
        if nu > nu_cut*(1+halo.z):
            ...
            for l in range(0,k):   #loop over energy
                ...
                xmatrix = np.tensordot(x,1.0/np.sin(theta_set),axes=0)
                theta_int = 0.5*np.sin(theta_set)**2*tools.int_bessel(xmatrix)  #theta integrand vectorised
...
def mass_sub_s(ms,mcut,mu):
    return ms**(mu)*np.exp(-(ms/mcut)**(-2.0/3))
...
def prob_sub_cs(cs,sigc,csbar):
    ...
    return 1.0/(np.sqrt(2.0*np.pi)*sigc*cs)*np.exp(-0.5*(np.log10(cs)-np.log10(csbar))**2/sigc**2)

def dns_sub(mass_set,mstar,mu):
    n = len(mass_set)
    dns = np.zeros(n,dtype=float)
    for i in range(0,n):
...
def delta(Mvir,fs,z,cos_env,dmmod,alpha):
    ...
    mass = np.zeros(mnum,dtype=float)
    cs = np.zeros(cnum,dtype=float)
    int_c = np.zeros(cnum,dtype=float)
    prob = np.zeros(cnum,dtype=float)
    deltams = np.zeros(mnum,dtype=float)

    for i in range(0,mnum):
        mass[i] = 10.0**(np.log10(mmin) + (np.log10(mmax) - np.log10(mmin))*i/(mnum-1))
    ...
    for i in range(0,mnum):
        ...
        cmax = 10.0**(np.log10(cbar) + 3*sigc)
        cmin = 10.0**(np.log10(cbar) - 4*sigc)
        for j in range(0,cnum):
            cs[j] = cmin + (cmax - cmin)*j/(cnum-1)
            prob[j] = prob_sub_cs(cs[j],sigc,cbar)
        norm = norm + tools.Integrate(prob,cs)
        for j in range(0,cnum):
            I1 = 1.0/astrophysics.rho_volume_int(cs[j],1.0,1,dmmod,alpha)/4.0/np.pi
            I2 = 1.0/astrophysics.rho_volume_int(cs[j],1.0,2,dmmod,alpha)/4.0/np.pi
...
def rb_norm_nfw(rb,rc,rho_s,rv,msm):
    ...
    a = rv*(rc - rb) + rb*(rv + rc)*np.log(rb/rc*(rv + rc)/(rb + rv))
    b = msm*(rb - rc)**2*(rv + rc)
    return 4.0*np.pi*rb*rc**3*rho_s*a/b - 1.0
...
def rb_norm(rb,rc,rho_s,r_set,msm,dmmod,alpha):
    ...
    int_set = 4.0*np.pi*r_set**2*rho*rho_s/msm/(1+r_set/rb)
...
def find_rb(rc,rho_s,rv,msm,dmmod,alpha):
    ...
    for i in range(0,n):
        r_set[i] = 10.0**(np.log10(rmin) + (np.log10(rv) - np.log10(rmin))*i/(n-1))
    rbmax = 100.0*rc
    rbmin = 0.0001*rc
...
def find_alpha(rv,rc,rho_s,rhoc,M):
    ...
    amin = 1e-3
    amid = 0.5*(amax + amin)
    fmax = 1.0/astrophysics.rho_volume_int(rv,rc,1,dmmod,alpha=amax)*rho_s*rhoc/M*4.0*np.pi - 1.0
    fmid = 1.0/astrophysics.rho_volume_int(rv,rc,1,dmmod,amid)*rho_s*rhoc/M*4.0*np.pi - 1.0
    fmin = 1.0/astrophysics.rho_volume_int(rv,rc,1,dmmod,amin)*rho_s*rhoc/M*4.0*np.pi - 1.0
    f = amid
    n = 0
