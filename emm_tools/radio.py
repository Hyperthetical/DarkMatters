...
def int_bessel_intp(t):
    ...
    Bessel integral approximation from interpolation function
        ...
        1D float array-like []
    """
    interpx = array([1e-4,1e-3,1e-2,3e-2,1e-1,2e-1,2.8e-1,3e-1,5e-1,8e-1,1.0,2.0,3.0,5.0,1e1])
    interpy = array([0.0996,0.213,0.445,0.613,0.818,0.904,0.918,0.918,0.872,0.742,0.655,0.301,0.130,2.14e-2,1.92e-4])
    spline = interp1d(interpx,interpy)
    result = -1*ones(len(t))
    result = where(t>1e1,sqrt(pi*0.5)*exp(-t)*sqrt(t),result)
    result = where(t<1e-4,4*pi/sqrt(3.0)/2.67894*(t*0.5)**(1.0/3),result)
    #print(t,result)
    if t[where(result==-1.0)[0]] != []:
        result = where(result==-1.0,spline(t),result)
    ...
    for i in range(0,num):  #loop over freq
        ...
        if nu > nu_cut*(1+halo.z):
            ...
            for j in range(0,n):  #loop over r
                ...
                nup = 8980.0*sqrt(ne)*1e-6    #plasma freq 
...
def radio_sb(nu_sb,halo,sim):
    ...
    print(emm_nu)
    if any(isnan(emm_nu)):
        nanIndex = abs(where(isnan(emm_nu))[0][0] - sim.n)
    else:
        nanIndex = 0
    for j in range(0,sim.n-nanIndex):
        ...
        for k in range(0,sim.n-nanIndex):    
            ...
            if(rprime >= r):
                ...
                lum[k] = emm_nu[k]*r/sqrt(r**2-rprime**2)
